...
export default function LoginPage() {
  const [state, formAction, pending] = useActionState(
    loginAction as unknown as (state: FormState, fd: FormData) => Promise<FormState>,
    null,
  );
    ...
    <div className="space-y-6">
      <div className="space-y-1 border-b border-ink-border pb-4">
        ...
        <p className="text-[10px] font-technical uppercase tracking-widest text-ink-muted m-0">
          Identifícate para entrar a tu área de trabajo
        </p>
      </div>
      ...
      <div className="border-t border-ink-border pt-4 flex items-center justify-between text-[10px] font-technical uppercase tracking-widest text-ink-muted">
        ...
        <Link href="/register" className="text-ink font-bold hover:text-highlight">
          Crear cuenta →
        </Link>
      </div>

      <p className="text-[9px] font-technical uppercase tracking-widest text-ink-muted text-center pt-2">
        Demo: admin / admin123
      </p>
    </div>
  );
